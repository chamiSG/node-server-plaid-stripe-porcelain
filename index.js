...
app.post('/api/create_link_token', function (request, response, next) {
  const configs = {
    user: {
      ...
      client_user_id: '60145ecdc08af90010c9f746',
    },
    client_name: 'Plaid Quickstart',
    products: PLAID_PRODUCTS,
    country_codes: PLAID_COUNTRY_CODES,
    language: 'en',
    webhook: 'https://198.12.255.51:80/webhook',
  };
...
app.post('/api/transferFunds', function (request, response, next) {
  // var token =    request.body.publictoken;
  // prettyPrintResponse(request.body.publictoken+"  "+request.body.accountid);

  // return response.json({
  //   token
      
  // });
  client.exchangePublicToken(request.body.publictoken, function (error, res) {
    
    var accessToken = res.access_token;
  // Generate a bank account token
  client.createStripeToken(accessToken, request.body.accountid, function(err, res) {
    var bankAccountToken = res.stripe_bank_account_token;
      const stripe = Stripe('sk_test_51HgdJaLs7sXosOlOyrMRxRaD73Y8Mv1BYtITPdJu24Z7LMPXLGE1CKoNjFuhBm1jO7h9Gww7qc1dfWA6jVNb9gKq00Mz2pfBz3');
